...
| `privacy_data_required` | `boolean` | **Yes** | Whether your plugin requires sensitive user data (e.g. location, name, email). Set `true` if it does, `false` if it does not. Drives PRISM minimization at the sandbox boundary. A future release will introduce field-level declaration (string array) for granular consent. |
...
### 3.5 Context field vocabulary (planned)

> **Status: coming soon.** A controlled vocabulary of user-context field names (e.g. `location`, `name`, `email`, `timezone`) will be published when field-level `privacy_data_required` declarations ship. Until then, use the boolean flag to indicate whether your plugin needs any user data.

### 3.6 Behavioral fields (planned)
...
- `privacy_data_required` set to `true` but the implementation does not actually use user data (PRISM violation).
...
1. **Context minimization:** Only plugins with **`privacy_data_required: true`** receive user context. The sandbox boundary (pre-deployed Cloudflare Workers at fixed URLs) enforces the allowlist.
...
All plugins use the boolean `privacy_data_required` field as described in [Section 3.1](#31-field-reference). Field-level declaration (specifying which data fields are needed) is planned for a future release.
